...
const History = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get(`/chat/questions?page=${currentPage}`);
        const { pairs: fetchedPairs, currentPage: fetchedCurrentPage, totalPages: fetchedTotalPages } = response.data;
  ...
  return (
    <div className="bg-gray-100 h-screen w-full">
      <nav className='h-16 flex items-center bg-white px-8 justify-between shadow-md border-b-gray-100 z-10'>
        <Link to='/'><h1 className='text-2xl font-bold'>Code<span className='text-blue-500'>Buddy</span></h1></Link>
        <Link to='/' className='flex items-center gap-1 md:gap-2 cursor-pointer text-[11px] md:text-sm bg-blue-100 text-blue-500 px-2 md:px-3 py-1 rounded-2xl hover:bg-blue-200'>
          <ArrowLeft size={16} />
          Back to Chat
        </Link>
      </nav>
      <div className='h-[calc(100%-64px)] flex flex-col w-full overflow-y-auto'>
        <div className='w-15/16 md:w-5/6 mx-auto p-4 h-full'>
          ...
          <p className='text-[14px] text-gray-500 space-x-4'>All your past coding questions</p>
          {isLoading ? (
            ...
            <div>
              ...
              {Object.keys(pairs).length === 0 ? (
                <div className="flex items-center justify-center h-40">
                  <p className="text-sm text-gray-500">You haven't asked any questions yet.</p>
                </div>
              ) : (
